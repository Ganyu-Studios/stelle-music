...
(
    async () => {
        ...
        if (!distExists) {
            ...
            console.error("\x1b[33m%s\x1b[0m", "The project needs to be built before it can be started.");
            console.error("\x1b[36m%s\x1b[0m", "\nPlease run: pnpm build\n");
            console.error("\x1b[90m%s\x1b[0m", "Or run 'pnpm clean' to build and clean in one step.");
            process.exit(1);
        }
