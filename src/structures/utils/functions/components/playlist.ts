...
export const PlaylistOps = {
     ...
     * @return {Promise<void>}
     */
    async save(ctx: CommandContext, interaction: ButtonInteraction, playlist: userPlaylist): Promise<void> {
        ...
        const embed = new Embed()
            ...
            .setDescription(messages.commands.playlist.manage.save.description)
            .setTimestamp();
     ...
     * @return {Promise<void>}
     */
    async toggle(ctx: CommandContext, interaction: ButtonInteraction, playlist: userPlaylist): Promise<void> {
        const { messages } = await ctx.locale();
     ...
     */
    async load(ctx: CommandContext, interaction: ButtonInteraction, playlist: userPlaylist): Promise<void> {
        ...
        if (bot && bot.channelId !== state.channelId)
            return interaction.editOrReply({
                ...
                embeds: [
                    {
                        description: messages.events.no.sharedVoice({ channelId: bot.channelId! }),
                        color: EmbedColors.Red,
                    },
                ],
            });
     ...
     * @returns {Promise<void>}
     */
    async remove(ctx: CommandContext, interaction: ButtonInteraction, playlist: userPlaylist): Promise<void> {
        const { messages } = await ctx.locale();
     ...
     * @returns {Promise<void>}
     */
    async info(ctx: CommandContext, interaction: ButtonInteraction, playlist: userPlaylist): Promise<void> {
        if (!ctx.inGuild()) return;
