...
function youtubeRadio(player: PlayerStructure, videoId: string, requester: TrackUser | null): Promise<TrackStructure[]> {
    return search(player, `https://www.youtube.com/watch?v=${videoId}&list=RD${videoId}`, SearchSources.YoutubeMusic, requester);
}
...
export async function seedMix(player: PlayerStructure, seed: TrackStructure, requester: TrackUser | null): Promise<Mix> {
    ...
    switch (sourceName) {
        case SourceNames.Spotify: {
            const mix: TrackStructure[] = await search(player, identifier, SearchSources.SpotifyTrackMix, requester);
            ...
            const [match]: TrackStructure[] = await search(player, `${title} ${author}`, SearchSources.Youtube, requester);
        ...
        case SourceNames.Deezer: {
            const recommendations: TrackStructure[] = await search(player, identifier, SearchSources.DeezerRecommendations, requester);
