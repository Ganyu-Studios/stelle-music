...
export const RadioOps = {
     ...
     */
    exclude(tracks: TrackStructure[], identifier: string): TrackStructure[] {
        return tracks.filter((track): boolean => track.info.identifier !== identifier);
    },
    /**
     *
     ...
     */
    radio(player: PlayerStructure, videoId: string, requester: TrackUser | null): Promise<TrackStructure[]> {
        return this.search({
            player,
            requester,
     ...
     */
    async mix(player: PlayerStructure, seed: TrackStructure, requester: TrackUser | null): Promise<Mix> {
        ...
        switch (sourceName) {
            case SourceNames.Spotify: {
                ...
                return { tracks: [], kind: MixKind.Radio };
            }

            case SourceNames.Youtube:
            case SourceNames.YoutubeMusic: {
                const radio: TrackStructure[] = await this.radio(player, identifier, requester);

                return { tracks: this.exclude(radio, identifier), kind: MixKind.Radio };
            }

            case SourceNames.Deezer: {
                const recommendations: TrackStructure[] = await this.search({
                    player,
                    requester,
                    query: identifier,
                ...
                if (recommendations.length) return { tracks: this.exclude(recommendations, identifier), kind: MixKind.List };
