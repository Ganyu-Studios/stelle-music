...
import { type APIButtonComponentWithCustomId, MessageFlags, ButtonStyle } from "seyfert/lib/types/index.js";

import { InvalidComponentRun, InvalidEmbedsLength, InvalidPageNumber, InvalidMessage } from "./Errors.js";
...
type StelleComponents = StelleStringMenu | StelleButton;
type StelleComponentInteraction = StringSelectMenuInteraction & ButtonInteraction;
...
export class EmbedPaginator {
    ...
    public async setPage(page: number): Promise<this> {
        if (!this.embeds.length) {
            throw new InvalidEmbedsLength("I can't send the pagination without embeds.");
        }
        if (!this.message) {
            throw new InvalidMessage("I can't set the page to an unresponded pagination.");
        }
     ...
     * @returns
     */
    public async edit(body: InteractionMessageUpdateBodyRequest | InteractionCreateBodyRequest): Promise<this> {
        if (!this.message) {
            throw new InvalidMessage("I can't set the page to an unresponded pagination.");
        }
    ...
    public reply(ephemeral = false): Promise<this> {
        ...
        return this.createCollector(ephemeral
     ...
     * @returns
     */
    public setRows(rows: ActionRow<StelleStringMenu | StelleButton>[]): this {
        this.rows = rows;
        return this;
    }
    ...
    private async createCollector(flags?: MessageFlags): Promise<this> {
        ...
        const collector = this.message.createComponentCollector({
            ...
            onStop: async (reason) => {
                if (reason === "idle") {
                    const components: ActionRow<StringSelectMenu | Button>[] = this.message!.components.map(
        ...
        collector.run(["pagination-pagePrev", "pagination-pageNext"], async (interaction) => {
            ...
            if (interaction.customId === "pagination-pageNext" && this.pages < this.embeds.length - 1) {
                ++this.pages;
            }
                ...
                for (const row of this.rows) {
                    for (const component of row.components) {
                        if ((component.data as { custom_id?: string }).custom_id === interaction.customId) {
                            const c = component as StelleComponents;
                            if (!("run" in c)) {
