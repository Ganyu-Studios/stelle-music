...
import { type PermissionNames, type PausedMode, StelleCategory } from "#stelle/types";
...
export default {
    ...
    messages: {
        commands: {
            ...
            setlocale: {
                invalidLocale: ({ locale, available }: { available: string } & ILocale) => `\`❌\` The locale : \`${locale}\` is invalid.\n\`📢\` **Available locales**: \`${available}\``,
                newLocale: ({ locale }: ILocale) => `\`✅\` The locale of **Stelle** is now: \`${locale}\``
            },
            ping: {
                message: "`🪶` Calculating...",
                response: ({ wsPing, clientPing, shardPing }: IPing) => `\`🌐\` Pong! (**Client**: \`${wsPing}ms\` - **API**: \`${clientPing}ms\` - **Shard**: \`${shardPing}ms\`)`
            },
            play: {
                ...
                embed: {
                    playlist: ({ playlist, tracks, volume, query, requester }: IPlayList) => `\`🎵\` The playlist [\`${playlist}\`](${query}) has been added to the queue.\n\n\`🔊\` **Volume**: \`${volume}%\`\n\`👤\` **Requested by**: <@${requester}>\n\`🔰\` **With**: \`${tracks} tracks\``,
                    result: ({ title, url, duration, volume, requester }: IPlayTrack) => `\`🎵\` Added [\`${title}\`](${url}) to the queue.\n\n\`🕛\` **Duration**: \`${duration}\`\n\`🔊\` **Volume**: \`${volume}%\`\n\`👤\` **Requested by**: <@${requester}>`,
                    results: ({ title, url, duration, volume, requester, position }: IPlayTrack) => `\`🎵\` Added [\`${title}\`](${url}) to the queue.\n\n\`🕛\` **Duration**: \`${duration}\`\n\`🔊\` **Volume**: \`${volume}%\`\n\`👤\` **Requested by**: <@${requester}>\n\n\`📋\` **Position in queue**: \`#${position}\``
                }
            },
            loop: {
            ...
            nodes: {
                value: ({ state, uptime, players }: INodes) => `\`📘\` State: \`${state}\`\n\`🕛\` Uptime: \`${uptime}\`\n\`🎤\` Players: \`${players}\``,
        ...
        events: {
            ...
            invalidOptions: ({ options, list }: IOptions) => `\`❌\` Invalid command options or arguments.\n-# - **Required**: \`<>\`\n-# - **Optional**: \`[]\`\n\n\`📋\` **Usage**:\n ${options}\n\`📢\` **Options Available**:\n${list}`,
            ...
            mention: ({ clientName, defaultPrefix, commandName }: IMention) => `\`📢\` Hey! My name is: **${clientName}** and my prefix is: \`${defaultPrefix}\` and **/** too!\n\`📋\` If you want to see my commands, type: \`${defaultPrefix} ${commandName}\` or /${commandName}.`,
            ...
            trackStart: {
                embed: ({ duration, requester, title, url, volume, author, size }: ITrackStart) => `\`📻\` Now playing [\`${title}\`](${url})\n\n\`🎤\` **Author**: \`${author}\`\n\`🕛\` **Duration**: \`${duration}\`\n\`🔊\` **Volume**: \`${volume}%\`\n\`👤\` **Requested by**: <@${requester}>\n\n\`📋\` **In queue**: \`${size} tracks\``,
                components: {
...
type IVoiceStatus = Pick<ITrackStart, "author" | "title">;
interface IHelp {
