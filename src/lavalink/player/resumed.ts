...
export default new Lavalink({
    ...
    run: async (client, node, _, players) => {
        ...
        for (const data of players) {
            ...
            if (data.track) {
                player.queue.current = client.manager.utils.buildTrack(data.track, session.me);
            }

            if (!player.queue.previous.length) {
                player.queue.previous.unshift(...session.queue!.previous);
            }
            if (!player.queue.tracks.length) {
                player.queue.add(session.queue!.tracks);
            }
            ...
            Object.assign(player, {
                ...
                paused: data.paused,
                playing: !data.paused && Boolean(data.track),
