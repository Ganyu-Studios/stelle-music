...
const { MatchOps } = await import("#stelle/utils/functions/internal/quiz.js");
...
for (const [guess, target, why] of ACCEPT) {
    test(`accepts "${guess}" ~ "${target}" (${why})`, (): void => {
        assert.equal(MatchOps.matches(guess, target), true);
    });
}

for (const [guess, target, why] of REJECT) {
    test(`rejects "${guess}" ~ "${target}" (${why})`, (): void => {
        assert.equal(MatchOps.matches(guess, target), false);
    });
}
